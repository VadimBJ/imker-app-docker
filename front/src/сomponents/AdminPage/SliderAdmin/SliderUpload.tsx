    ...
    if (imageData && selectedFile) {
      ...
      if (imageData && selectedFile) {
        ...
        try {
          const response = await axios.post(
            `/api/files/upload?width=${width}&height=${height}&category=SLIDER`,
            formData,
            {
      ...
      if (linkVar) {
        try {
          ...
          toast.success("Your slider has been successfully sent!", {
          ...
          console.error(
            "There was an error when sending a slider data to Back:",
            error
          );
        }
      }
